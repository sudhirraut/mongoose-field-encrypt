...
const fieldEncryption = function (schema, options) {
  ...
  let secret = useAes256Ctr ? options.secret : hash.digest("hex").substring(0, 32);
  ...
  function decryptFieldsArray(arr, fields, secret) {
    for (const field of fields) {
      const encryptedFieldName = encryptedFieldNamePrefix + field;
      const encryptedFieldData = encryptedFieldName + encryptedFieldDataSuffix;
  ...
  function updateHook(_next) {
    ...
    if (this.options && this.options.__secret__) {
      secret = this.options.__secret__;
    }
  ...
  schema.methods.decryptFieldsSync = function (options) {
    if (options && options.__secret__) {
      secret = options.__secret__;
    }
    decryptFields(this, fieldsToEncrypt, secret);
  };

  schema.methods.encryptFieldsSync = function (options) {
    if (options && options.__secret__) {
      secret = options.__secret__;
    }
  ...
  //       console.log('************************* >>>>> key = ', key, ", Data =", jsonObj[key]);
  //     }
  //   const next = getCompatitibleNextFunc(_next);
  ...
  schema.post("find", function (_data, _next) {
    const next = getCompatitibleNextFunc(_next);
    const data = getCompatibleData(_next, _data);
    ...
    try {
      decryptFieldsArray(data, fieldsToEncrypt, secret);      
      next();
    } catch (err) {
      console.log('#######>>> ', err);
      next(err);
  ...
  schema.post("findOne", function (_data, _next) {
    const next = getCompatitibleNextFunc(_next);
    const data = getCompatibleData(_next, _data);
  ...
  schema.pre("save", function (_next) {
    if (this.$__.saveOptions && this.$__.saveOptions.__secret__) {
      secret = this.$__.saveOptions.__secret__;
    }
    const next = getCompatitibleNextFunc(_next);
    try {
      encryptFields(this, fieldsToEncrypt, secret);
